...
parser.add_argument('--pseudo_label_threshold', type=float, default=0.8)
...
#def adapt_epoch(model_t, model_s, optim, train_loader, args, feature_bank, pred_bank, loss_weight=None, model_ema_rate=None):
def adapt_epoch(model_t, model_s, optim, train_loader, args, feature_bank, pred_bank, loss_weight=None, model_ema_rate=None):
    for sample_w, sample_s in train_loader:
        ...
        #a = torch.sum(predictions_stu_s/(8*2*512*512))
        #b = torch.sum(predictions_tea_w/(8*2*512*512))
        a = torch.mean(predictions_stu_s_sigmoid)
        b = torch.mean(predictions_tea_w_sigmoid)
        
        ratio = 1 - abs(a-b)
        
        # ratio = abs(a/b)
        if ratio > 1:
            model_ema_rate = model_ema_rate
        else:
            model_ema_rate = ratio
        ...
        t_out = F.softmax(predictions_tea_w_sigmoid, dim=-1)
        s_out = F.softmax(predictions_stu_s_sigmoid, dim=-1)
        mean_out = F.softmax((t_out + s_out) * 0.5, dim=-1)
        kl_loss = torch.nn.KLDivLoss(reduction='batchmean')
        loss2 = 0.0001 * (kl_loss(torch.log(s_out), mean_out) + kl_loss(torch.log(t_out), mean_out)) / 2
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(data_loader):
            ...
            for idx in range(len(sample['img_name'])):
                ...
                cv2.imwrite(f'E:/code/Medical Image/SFDA-CBMT/results/constrast/{path}/{disc_name}', disc)
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(data_loader):
            ...
            for idx in range(len(sample['img_name'])):              
                ...
                cv2.imwrite(f'E:/code/Medical Image/SFDA-CBMT/results/constrast/{path}/{cup_name}', cup)
...
def main():
    ...
    task_name = f'dynaminc_constrast-{args.pseudo_label_threshold}-ratio-{args.mean_loss_calc_bound_ratio}'
